...
# ==============================================================================
# 1. CONFIGURACIÓN DEL SISTEMA Y DISEÑO VISUAL (UI/UX)
# ==============================================================================
st.set_page_config(
...
# INYECCIÓN DE ESTILOS CSS (DISEÑO V8 - CLARO Y POTENTE)
st.markdown("""
    <style>
    /* 1. ANIMACIÓN DE PULSO PARA AMENAZAS CRÍTICAS (Latido de Alerta) */
    @keyframes pulse-red {
    ...
    /* 2. TARJETAS DE ESTADO (Diseño que te gustó) */
    
    /* CRÍTICO: Fondo rojo oscuro, borde rojo brillante, animado */
    .threat-box-critical {
    ...
    /* ADVERTENCIA: Fondo ámbar oscuro, borde naranja */
    .threat-box-warning {
    ...
    /* SISTEMA: Fondo púrpura (Aprendizaje) */
    .system-box {
...
# ==============================================================================
# 2. NÚCLEO DE MEMORIA DINÁMICA (CEREBRO EVOLUTIVO)
# ==============================================================================
class MemoryCore:
    """
    ...
    """
    def __init__(self):
        # Memoria de Identidad (Quién eres)
        self.user_name = None
        self.user_trust_level = 50 # Nivel de confianza inicial
        
        # Memoria de Conocimiento (Qué ha aprendido)
    ...
    def set_identity(self, name):
        """Registra el nombre del usuario y mejora la personalización."""
        self.user_name = name.strip().capitalize()
        
    def get_greeting_context(self):
        """Genera un saludo dinámico basado en si conoce al usuario."""
        hour = datetime.datetime.now().hour
        time_greeting = "Buenos días" if 5 <= hour < 12 else "Buenas tardes" if 12 <= hour < 19 else "Buenas noches"
    ...
    def learn_new_threat(self, phrase, reason):
        """
        Permite a la IA reprogramarse a sí misma añadiendo nuevas reglas regex
        basadas en la enseñanza del usuario.
        """
        ...
        new_rule_structure = {
            ...
            "peso": 100, # Prioridad máxima porque el usuario lo ordenó
            "cat": "AMENAZA APRENDIDA",
            "desc": f"El usuario me enseñó explícitamente que '{phrase}' es peligroso.",
            "consejo": f"Bloqueo activado por protocolo personalizado. Razón: {reason}"
        }
...
# ==============================================================================
# 3. MOTOR DE PROCESAMIENTO LINGÜÍSTICO (NLP)
# ==============================================================================
class LinguisticEngine:
    """
    Se encarga de limpiar, normalizar y preparar el texto para que ningún hacker
    pueda evadir los filtros usando trucos de escritura.
    """
    def __init__(self):
        # Diccionario masivo de Leetspeak (Hacker -> Español)
        self.leetspeak_map = {
    ...
    def normalize_deeply(self, text):
        ...
        # 3. Decodificación de Leetspeak caracter por caracter
        decoded_text = ""
        for char in text:
            decoded_text += self.leetspeak_map.get(char, char)
            
        # 4. Limpieza de caracteres no alfanuméricos (mantiene espacios)
    ...
    def is_english(self, text):
        """Detecta si el texto está mayoritariamente en inglés."""
        ...
        # Si más del 20% son palabras clave en inglés
        return ratio > 0.20

# ==============================================================================
# 4. BASE DE DATOS DE AMENAZAS (EL CEREBRO ESTÁTICO)
# ==============================================================================
class ThreatRepository:
    """
    Contiene el conocimiento base "Militar" pre-programado.
    Estas reglas son inmutables, pero se complementan con la memoria dinámica.
    """
    def __init__(self):
        self.static_rules = {
            # --- NIVEL 1: INTENCIÓN DE LUCRO (EL MÁS IMPORTANTE) ---
            "INTENCION_DINERO": {
                ...
                "peso": 95,
                "cat": "EXTORSIÓN FINANCIERA",
                "desc": "El remitente está solicitando recursos económicos explícitamente.",
                "consejo": "ALERTA MÁXIMA. No envíes dinero bajo ninguna circunstancia."
            },
            
            # --- NIVEL 2: ROBO DE CREDENCIALES ---
            "DATOS_SENSIBLES": {
                "regex": r"(tarjeta|visa|mastercard|amex|cvv|pin|nip|cuenta bancaria|clabe|numero de cuenta|banco|plastico|vencimiento|digitos|contraseña|password|usuario)",
                "peso": 85,
                "cat": "ROBO DE DATOS",
                "desc": "Solicitud de información bancaria o credenciales de acceso.",
                "consejo": "Tu banco o servicios legítimos NUNCA piden estos datos por chat."
            },
            
            # --- NIVEL 3: PLATAFORMAS DE MOVIMIENTO RÁPIDO ---
            "PLATAFORMAS_PAGO": {
                ...
                "desc": "Mención de plataformas usadas por estafadores para mover dinero rápido.",
                "consejo": "Verifica la identidad del receptor por voz antes de transferir."
            },
            
            # --- NIVEL 4: INGENIERÍA SOCIAL (MIEDO/URGENCIA) ---
            "TACTICAS_MIEDO": {
            ...
            "ENLACES_PELIGROSOS": {
                ...
                "desc": "Intento de redirección a sitios web fraudulentos.",
                "consejo": "No hagas clic. Podrían robar tus cookies o infectar tu dispositivo."
            },
            ...
            "SECUESTRO_WHATSAPP": {
                ...
                "desc": "Intento crítico de robo de cuenta de WhatsApp o acceso remoto.",
                "consejo": "JAMÁS compartas el código SMS. Bloquea inmediatamente."
            }
        }
...
class OianOmniscientBrain:
    def __init__(self):
        ...
        # Conexión con Session State para persistencia de memoria
    ...
    def _detect_commands(self, text):
        ...
        # 1. COMANDO: IDENTIDAD
        match_identity = re.search(r"\b(me llamo|soy|mi nombre es)\s+([a-zA-ZáéíóúÁÉÍÓÚñÑ]+)", text_lower)
        if match_identity:
        ...
        if match_learn:
            ...
            reason = "Instrucción directa del usuario"
            self.memory.learn_new_threat(phrase, reason)
            return {
                "type": "LEARNING",
                "msg": f"He reescrito mi código neuronal. Ahora detectaré **'{phrase}'** como una amenaza crítica.",
    ...
    def analyze_message(self, input_text):
        ...
        if self.linguist.is_english(input_text):
            return {
                "status": "ERROR_IDIOMA",
                "msg": "He detectado idioma INGLÉS. Por protocolos de seguridad y precisión, solo analizo Español.",
                "score": 0, "threats": []
            }

        # Paso 3: Construcción de Base de Datos Unificada (Estática + Aprendida)
        ...
        for rule_id, rule_data in active_rules.items():
            ...
            if count > 0:
                # Cálculo de daño: Peso base + 10% por cada repetición
        ...
        # Normalizamos el score a máx 100
        final_score = min(int(total_risk_score), 100)
        
        # Paso 5: Lógica Social y Contextual (Si no hay amenazas)
        is_social = False
        social_response = ""
        ...
        if final_score == 0:
            if re.search(r"(hola|buenos dias|hey|que tal)", text_norm):
                ...
                social_response = f"👋 **{greeting}** Mis sensores están operativos y esperando datos para analizar."
                ...
                social_response = f"🛡️ A la orden, {name_ref}. La vigilancia es constante."
            elif re.search(r"(hambre|comida|sueño|dormir|cansado|triste)", text_norm):
                is_social = True
                social_response = "🧠 **Nota Psicológica:** Los estafadores suelen atacar cuando detectan fatiga o vulnerabilidad emocional. Cuídate primero."
...
# ==============================================================================
# 6. INTERFAZ GRÁFICA DE USUARIO (FRONTEND)
# ==============================================================================
    ...
    if rules_count > 0:
        with st.expander("Ver Conocimiento Adquirido"):
            for k in st.session_state.memory_core.learned_knowledge:
                ...
                st.code(f"🚫 {clean_name}")

    st.markdown("---")
    if st.button("🗑️ FORMATEAR SISTEMA"):
        st.session_state.chat_history = []
...
st.title("🛡️ SISTEMA DE DEFENSA EVOLUTIVO")
st.markdown("""
<div style='background-color:rgba(0,0,0,0.2); padding:10px; border-radius:10px; border-left: 4px solid #3b82f6; margin-bottom:20px;'>
    ...
    1. Para análisis: Pega el mensaje sospechoso.<br>
    2. Para identidad: Escribe <em>"Me llamo [Nombre]"</em>.<br>
    3. Para enseñar: Escribe <em>"Aprende que [Palabra] es estafa"</em>.
</div>
""", unsafe_allow_html=True)
...
if user_input:
    ...
    st.session_state.chat_history.append({"role": "user", "avatar": "👤", "content": user_input})
    ...
    with st.chat_message("assistant", avatar="👁️"):
        ...
        with st.status("🧠 Accediendo a sinapsis neuronales...", expanded=False) as status:
            time.sleep(0.3)
            st.write("🔍 Normalizando texto y rompiendo encriptación...")
            time.sleep(0.2)
            st.write("📂 Consultando base de datos estática...")
            time.sleep(0.2)
            st.write("💾 Verificando memoria aprendida...")
            status.update(label="Análisis Completado", state="complete")

        # Ejecución del Cerebro
        result = st.session_state.brain.analyze_message(user_input)
        
        # 3. GENERACIÓN DE RESPUESTA VISUAL (HTML/CSS)
        html_content = ""
        
        # --- CASO 1: APRENDIZAJE DE SISTEMA (PÚRPURA) ---
        if result["status"] == "SYSTEM":
            html_content = f"""
            <div class="system-box">
                <h2 class="card-title">💾 ACTUALIZACIÓN DE SISTEMA</h2>
                <p class="card-text">{result['system_msg']}</p>
                <p class="card-text"><i>{result['sub_msg']}</i></p>
            </div>
            """
            
        # --- CASO 2: AMENAZA CRÍTICA (ROJO PULSANTE) ---
        elif result["status"] == "CRITICAL":
            threats_str = ", ".join([f"<span class='highlight-threat'>{t}</span>" for t in result['threats']])
            
            # Extraemos el consejo más importante (el del primer reporte)
            main_advice = result['report'][0]['consejo'] if result['report'] else "Bloquea al usuario."
            
            html_content = f"""
            <div class="threat-box-critical">
                <h2 class="card-title">🚨 AMENAZA CRÍTICA ({result['score']}%)</h2>
                <p class="card-text"><strong>ANÁLISIS:</strong> El mensaje contiene patrones de ataque confirmados.</p>
                <p class="card-text"><strong>VECTORES DETECTADOS:</strong> {threats_str}</p>
                <hr style="border-color: rgba(255,255,255,0.3);">
                <p class="card-text" style="font-size: 18px; font-weight:bold;">🛡️ PROTOCOLO DE ACCIÓN: {main_advice}</p>
            </div>
            """
            
        # --- CASO 3: ADVERTENCIA (ÁMBAR) ---
        elif result["status"] == "WARNING" or result["status"] == "CAUTION":
            threats_str = ", ".join(result['threats'])
            html_content = f"""
            <div class="threat-box-warning">
                <h2 class="card-title">⚠️ ALERTA DE SEGURIDAD ({result['score']}%)</h2>
                <p class="card-text">Se han detectado elementos sospechosos.</p>
                <p class="card-text"><strong>PATRONES:</strong> {threats_str}</p>
                <hr style="border-color: rgba(255,255,255,0.3);">
                <p class="card-text">No realices ninguna acción precipitada. Verifica la fuente.</p>
            </div>
            """
        ...
        # --- CASO 4: ERROR IDIOMA ---
            ...
            html_content = f"""
            <div class="threat-box-warning">
                <h2 class="card-title">🇺🇸 IDIOMA DETECTADO</h2>
        ...
        # --- CASO 5: SOCIAL (AZUL) ---
            ...
            <div class="social-box">
                <h2 class="card-title">💬 ENLACE SOCIAL ESTABLECIDO</h2>
        ...
        # --- CASO 6: SEGURO (VERDE) ---
            ...
            safe_msg = "El escaneo heurístico no encontró amenazas conocidas en mis bases de datos."
            
            html_content = f"""
            <div class="safe-box">
                <h2 class="card-title">✅ ANÁLISIS SEGURO (0%)</h2>
                <p class="card-text">{safe_msg}</p>
                <br>
                <p style="font-size:12px; opacity:0.8;"><i>Nota: Si crees que me equivoqué, enséñame escribiendo: "Aprende que [palabra] es estafa".</i></p>
            </div>
            """
        ...
        st.session_state.chat_history.append({"role": "assistant", "avatar": "👁️", "content": html_content})